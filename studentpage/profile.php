...
<body>
	...
	<header class="header">
		...
		<i class='bx bxs-user-account bx-burst' style='color:#f9f4f4' ></i><span style="right;color:white;"> تغيير كلمة السر  </span>
		</a>
	</header>
	<div class="body">
		<nav class="side-bar">
		...
		<div class="main-container">
			...
			if(isset($_POST['menu_ar_change_pass'])){
				...
				?>
				<div class="modal fade" id="validate_pass" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
				<div class="modal-header">
				  <h5 class="modal-title" id="exampleModalCenterTitle"><h3>كلمة السر غير صحيحة</h3></h5>
				  <button type="button" class="close" onclick="goBack()" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				  </button>
				</div>
				<div class="modal-body">
				<form class="contact-form "  action="?change_pass=" method="post" id="" enctype="multipart/form-data">
			...
			?>
			<div class="modal fade" id="validate_new_pass" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
			<div class="modal-header">
			  <h5 class="modal-title" id="exampleModalCenterTitle"><h3>كلمة السر الجديدة غير متطابقة</h3></h5>
			  <button type="button" class="close" onclick="goBack()" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">&times;</span>
			  </button>
			</div>
...
<div class="modal fade" id="change_pass" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
 <div class="modal-dialog modal-dialog-centered" role="document">
   <div class="modal-content">
	 ...
	 <div class="modal-body">
	   ...
	   <form class="contact-form "  action="profile.php" method="post" id="" enctype="multipart/form-data">
 ...
 ?>
  ...
  <div class="card-body">
			...
			<form class="form-horizontal" action="profile.php" method="post" style="background-color: azure;">
  
			 <span style="font-size:23px; color:#6c757d;">رقم القيد :<?php echo $_SESSION['user_id'] ?></span>
			 <br><br><br>
			 <fieldset>   
				<?php  
...
?>					
						 ...
						 <div class="form-group ">
						  ...
						  <input type="text" name="fName" class="col-xs-2 col-sm-2 col-md-2 text-right" disabled placeholder="الاسم الاول*"  required=""  value="<?php echo ''.$user['student_name'].''?>" >
						   <label for="" class="control-label">إسم الاب*</label>
						  <input type="text" name="mName" class="col-xs-2 col-sm-2 col-md-2 text-right" disabled  placeholder="إسم الاب*"  required="" value="<?php echo ''.$user['mname'].''?>">
							   <label for="" class="control-label">  اللقب* </label>
						  <input type="text" name="lName" class="col-xs-2 col-sm-2 col-md-2 text-right" disabled placeholder=" اللقب*"  required=""  value="<?php echo ''.$user['lname'].''?>">
					  </div>
				  <fieldset>
			   ...
			   <fieldset>
					...
					<input class="col-xs-2 col-sm-2 col-md-2 " type="date" value="<?php echo date('Y-m-d',strtotime($user["birthday"])) ?>" name="bday" id="bday">
					<?php 
					} 
					else { 
						?>
							<input class="col-xs-2 col-sm-2 col-md-2 " type="date" value="<?php echo date('Y-m-d',strtotime($user["birthday"])) ?>" disabled name="bday" id="bday">
					<?php
					} 
					?>
					<label class="control-label" for="bplace">مكان الميلاد</label>
					<?php if($user['place_birth']==''or $user['place_birth']==null){?>
					<input class="col-xs-2 col-sm-2 col-md-2 " id="bplace" name="bplace" type="text" placeholder="مكان الميلاد"  required="" value="<?php echo ''.$user['place_birth'].''?>">
					<?php }else{?>
					<input class="col-xs-2 col-sm-2 col-md-2 " id="bplace" name="bplace" type="text" disabled placeholder="مكان الميلاد"  required="" value="<?php echo ''.$user['place_birth'].''?>">
					<?php
				}	?>	
					<label class="control-label" for="gender">البلد</label>
					<?php if($user['country']==''or $user['country']==null){?>
				   <input class="col-xs-2 col-sm-2 col-md-2 " name="cuntid" id="cuntid" type="text" placeholder="البلد" required=""  value="<?php echo ''.$user['country'].''?>">
				<?php }
				else{ ?>
					<input class="col-xs-2 col-sm-2 col-md-2 " name="cuntid" id="cuntid" type="text" placeholder="البلد" required="" disabled  value="<?php echo ''.$user['country'].''?>">
				<?php } ?>
				</div>
			   </fieldset>
			   <fieldset>
		...
		<input class="col-xs-2 col-sm-2 col-md-2" name="gender" id="gender" required="" placeholder="الجنس" value="<?php echo ''.$user['gender'].''?>">
	  <?php }
	  else { ?>
		<input class="col-xs-2 col-sm-2 col-md-2" name="gender" id="gender" required="" disabled placeholder="الجنس" value="<?php echo ''.$user['gender'].''?>">
		<?php } ?>
		<label class="control-label" for="status">الحالة الاجتماعية</label>
		<?php if($user['marital_status']==''or $user['marital_status']==null){ ?>			
		<input class="col-xs-2 col-sm-2 col-md-2" name="status" id="status" required="" placeholder="الحالة الإجتماعية" value="<?php echo ''.$user['marital_status'].''?>">				   
		<?php }
		else{ ?>
			<input class="col-xs-2 col-sm-2 col-md-2" name="status" id="status" required="" placeholder="الحالة الإجتماعية" disabled value="<?php echo ''.$user['marital_status'].''?>">				   			
		<?php }	?>		
		<label class="control-label" for="nationality">الجنسية</label> 
		<?php if($user['Nationality']==''or $user['Nationality']==null){ ?>	
		<input class="col-xs-2 col-sm-2 col-md-2" name="natid" id="natid" required="" placeholder="الجنسية" value="<?php echo ''.$user['Nationality'].''?>">
		<?php }
		else{ ?>
			<input class="col-xs-2 col-sm-2 col-md-2" name="natid" id="natid" required="" placeholder="الجنسية" disabled value="<?php echo ''.$user['Nationality'].''?>">
		<?php
		}	?>		
		</div>
		  </fieldset>
	 ...
	 <fieldset>
						  ...
						  <div class="form-group">
						 <label class="control-label" for="age">الرقم الوطنى أو جواز السفر للمغتربين*</label>
						 <?php if($user['identity']==''or $user['identity']==null){ ?>	
							<input class="col-xs-2 col-sm-2 col-md-2" id="nid" name="nid" type="text" placeholder="الرقم الوطنى أو جواز السفر للمغتربين*" maxlength="12" required="" value="<?php echo ''.$user['identity'].''?>">
						   <?php
						 }
						 else {
							?>
							<input class="col-xs-2 col-sm-2 col-md-2" id="nid" name="nid" type="text" placeholder="الرقم الوطنى أو جواز السفر للمغتربين*" disabled maxlength="12" required="" value="<?php echo ''.$user['identity'].''?>">
						<?php
						}
							?>
						  <label class="control-label" for="contact">رقم الهاتف*</label>
						  <?php if($user['phone']==''or $user['phone']==null){ ?>								
							<input class="col-xs-2 col-sm-2 col-md-2" id="contact" name="contact" type="tel" placeholder="رقم الهاتف*" maxlength="10" required="" pattern="09.+"  value="<?php echo ''.$user['phone'].''?>">
						   <?php
						  }
						  else{
							?>
							<input class="col-xs-2 col-sm-2 col-md-2" id="contact" name="contact" type="tel" placeholder="رقم الهاتف*" disabled maxlength="10" required="" pattern="09.+"  value="<?php echo ''.$user['phone'].''?>">
							<?php
						  }
						  ?>
						  ...
						  <?php if($user['email']==''or $user['email']==null){ ?>								
							...
							<input class="col-xs-2 col-sm-2 col-md-2" id="email" name="email" type="email" placeholder="البريد الالكتروني*" required=""  value="<?php echo ''.$user['email'].''?>"
						  <?php
						  }
						  else {
							?>
							<input class="col-xs-2 col-sm-2 col-md-2" id="email" name="email" type="email" placeholder="البريد الالكتروني*" disabled required=""  value="<?php echo ''.$user['email'].''?>"
							<?php
			   ...
			   <fieldset>
						...
						<div class="form-group">
						 ...
						 <input class="col-xs-2 col-sm-2 col-md-2" id="addhome" name="addhome" type="text" placeholder="عنوان السكن"  value="<?php echo ''.$user['address'].''?>">
							  
							<?php
						 }
						else{
							?>
							<input class="col-xs-2 col-sm-2 col-md-2" id="addhome" name="addhome" type="text" placeholder="عنوان السكن" disabled  value="<?php echo ''.$user['address'].''?>">
						 <?php
						 } 
						 ?>
						...
						if($user['city']==''or $user['city']==null){ ?>								
							<input class="col-xs-2 col-sm-2 col-md-2" id="city" name="city" type="text" placeholder="المدينة"  value="<?php echo ''.$user['city'].''?>">
						   <?php
						}
						else {
							?>
														<input class="col-xs-2 col-sm-2 col-md-2" id="city" name="city" type="text" placeholder="المدينة" disabled  value="<?php echo ''.$user['city'].''?>">
						<?php
						}
						?>
					</div>
			  ...
			  <table>
					  <tbody><tr><td>
						...
						and $user['city']!='' or $user['city']!=null ){
							?>
							<input type="submit" id="submit" class="btn btn-primary" disabled value="حفظ" name="edit" >
						<?php
						}
						else{
							?>
						 <input type="submit" id="submit" class="btn btn-primary" value="حفظ" name="edit" >
						<?php
						} ?>
						 </td>
