...
<head>
	...
	<title>عرض المواد المنجزة</title>
...
<table class="table">
	...
	JOIN courses ON courses.course_id=student_has_courses.course_id WHERE student.student_id='$student_id' ";
 ...
 if($rows>0){
	while($row = $result->fetch_array(MYSQLI_ASSOC)){	
		...
		';
			$count+=$row['number_units'];
			$sum+=$row['result']*$row['number_units'];	
    }
else {
    continue;
	}
  ...
  <tfooter>
	...
	$avg=$sum/$count;
	?>
	<tr style="background-color: rgba(0,123,255,.25);">
	<th scope="row"><?php echo $num++ ?></th>
	<?php 
	if($avg>=50){
		?>
	<td style="color:#28a745;" colspan="2"><b><?php echo $avg; ?></b></td>
<?php	
}
	else{
		?>
		<td style="color:red;" colspan="2"><b><?php echo $avg; ?></b></td>
		<?php
	}
	?>
	<td colspan="2"><b><?php echo "  :متوسط هذا الفصل"; ?></b></td>
	</tr>
</tfooter>
<?php
}
